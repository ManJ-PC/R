...
# Asneira, ..mostrou somente a legenda .. 

#e mostra um subconjunto em que o país vai ser igual  a Portugal
...
top_countries <- s1 %>%  # dataset
  group_by(country) %>% 
  summarise(total_emissions = sum(total_CO2, na.rm = TRUE)) %>%
  arrange(desc(total_emissions))

# <=> a summarise(group_by(s1, country), total_emissions = sum(total_CO2, na.rm = T))
...
# countries per year
total_CO2 <- s1 %>%
  group_by(year) %>%   # Varios passos feitos por uma pipeline!
  summarise(
