...
---
Dedicado à ideia de simulação (laboratório de estatística e DS) em forma de ... consequências e limitações d eum procedimento e o seu potencial. Mecanismos de (omissão de dados)... e simular dados
...
Distribuição em forma de símbolo..
...
require(MASS) # lê um pacote, permite ver visualização/distribuição em forma de sino
```
...
\end{pmatrix}.
\]
We can now generate the data, which we do as illustrated below. I will consider $n=100$ and will also fix the seed (for the random number generator), so that the results are reproducible (i.e., we will all obtain the same values). Vetor média e vetor de covariância! E correlação de 0.6!  
